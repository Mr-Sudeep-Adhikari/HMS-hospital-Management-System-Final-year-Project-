...
<head>
  ...
  <title>Appointment Details | KMC Hospital</title>
...
<body class="gradient-bg min-h-screen text-gray-800 p-6">
  ...
  if (isset($_POST['app_search_submit'])) {
    ...
    if ($result->num_rows == 0) {
      ...
      echo "<div class='max-w-7xl mx-auto mt-20 animate-fade-in-up'>
        <div class='card rounded-3xl shadow-2xl overflow-hidden'>
            <div class='bg-emerald-100 p-8 border-b border-emerald-200 flex justify-between items-center'>
                <h2 class='text-3xl font-extrabold text-emerald-800'>Appointment Search Results</h2>
            ...
            <div class='p-8 overflow-x-auto'>
                <table class='w-full text-left border-collapse'>
                    <thead class='bg-emerald-50 text-emerald-800'>
                        <tr>
                            <th class='px-6 py-4 rounded-l-xl'>Patient Name</th>
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 1)) {
          $appstatus = "<span class='text-green-600 font-bold'>Active</span>";
        }
        if (($row['userStatus'] == 0) && ($row['doctorStatus'] == 1)) {
          $appstatus = "<span class='text-red-600 font-bold'>Cancelled by Patient</span>";
        }
        if (($row['userStatus'] == 1) && ($row['doctorStatus'] == 0)) {
          $appstatus = "<span class='text-orange-600 font-bold'>Cancelled by Doctor</span>";
        }

        echo "<tr class='border-b hover:bg-emerald-50 transition'>
                <td class='px-6 py-4 font-bold'>$fname $lname</td>
                <td class='px-6 py-4'>$email</td>
                <td class='px-6 py-4'>$contact</td>
                <td class='px-6 py-4'>$doctor</td>
                <td class='px-6 py-4'>$docFees</td>
                <td class='px-6 py-4'>$appdate</td>
                <td class='px-6 py-4'>$apptime</td>
                <td class='px-6 py-4'>$appstatus</td>
            </tr>";
      }
